...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		boyfriend = new Boyfriend(450, 380, "sex");
		...
		Application.current.window.title = Main.appTitle + ' - ' + HelperFunctions.getArtist(SONG.song.toLowerCase()) + ' - ' + StringTools.replace(SONG.song.toUpperCase(), " ", "-") + '';
		...
		if (isStoryMode)
		{
			doof = new DialogueBox(CoolUtil.coolTextFile(Paths.txt('data/dialogueFUCK')));
			// doof.x += 70;
        ...
        healthBar.createFilledBar(FlxColor.fromRGB(255,134,61), FlxColor.fromRGB(84,155,149));
        // healthBar
		add(healthBar);
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
...

			{
				...
				case 2:
					...
					var creditTxt:FlxText = new FlxText(4,healthBarBG.y + 20,0,("opt by fisk skr"), 24);
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.pressed.CONTROL && FlxG.keys.justPressed.ONE)
		{
			endSong();
		}
		...
		if (controls.PAUSE && startedCountdown #if android || FlxG.android.justReleased.BACK #end && canPause && !cannotDie)
		{
	...
	function endSong():Void
	{
		...
		MainMenuState.throwCut = false;

		if (isStoryMode && SONG.song.toLowerCase() == 'perfection')
		{
			MainMenuState.throwCut = true;
		}
   ...
   private function keyShit():Void // I've invested in emma stocks
	{
		...
		if (!PlayStateChangeables.botPlay)
		{
			playerStrums.forEach(function(spr:StaticArrow)
			{
				if (pressArray[spr.ID] && spr.animation.curAnim.name != 'confirm' && spr.animation.curAnim.name != 'pressed' && !spr.animation.curAnim.name.startsWith('dirCon'))
	...
	function goodNoteHit(note:Note, resetMashViolation = true):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			boyfriend.playAnim('sing' + dataSuffix[note.noteData] + altAnim, true, false, delay);
